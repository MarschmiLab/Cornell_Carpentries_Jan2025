...
> ## Solution
> ```{r,comment=NA,echo=F,collapse=T}
> filter(gapminder, year==1982) %>% slice_max(cells_per_ml)
> ```
{: .solution}
...
> ## Solution
> ```{r,comment=NA,echo=F,collapse=T}
> gapminder %>% filter(env_group=='Asia') %>% ggplot(aes(x=as.character(year),y=cells_per_ml)) +
> geom_boxplot() +
...
> ## Solution
> ```{r,comment=NA,echo=F,collapse=T}
> gapminder %>% ggplot(aes(x=as.character(year),y=cells_per_ml)) +
> geom_boxplot() +
...
> ## Solution
> ```{r,comment=NA,echo=F,collapse=T}
> filter(gapminder, year == 1952 | year == 2007) %>% select('country','year',cells_per_ml) %>% pivot_wider(names_from=year,values_from=cells_per_ml) %>% mutate(diff=`2007`-`1952`) %>% slice_max(diff)
> ```
{: .solution}
...
> ## Solution
> ```{r,comment=NA,echo=F,collapse=T}
> filter(gapminder, year == 1952 | year == 2007) %>% select('country','year',cells_per_ml) %>% pivot_wider(names_from=year,values_from=cells_per_ml) %>% mutate(diff=`2007`-`1952`) %>% filter(diff < 0)
> ```
{: .solution}
