...
> ## Solution
> ```{r,comment=NA,echo=F,collapse=T}
> filter(sample_and_taxon, temperature==1982) %>% slice_max(cells_per_ml)
> ```
{: .solution}
...
> ## Solution
> ```{r,comment=NA,echo=F,collapse=T}
> sample_and_taxon %>% filter(env_group=='Asia') %>% ggplot(aes(x=as.character(temperature),y=cells_per_ml)) +
> geom_boxplot() +
...
> ## Solution
> ```{r,comment=NA,echo=F,collapse=T}
> sample_and_taxon %>% ggplot(aes(x=as.character(temperature),y=cells_per_ml)) +
> geom_boxplot() +
> facet_wrap(~env_group) +
> theme_bw() +
> theme(axis.text.x=element_text(angle=90)) +
...
> ## Solution
> ```{r,comment=NA,echo=F,collapse=T}
> filter(sample_and_taxon, temperature == 1952 | temperature == 2007) %>% select('country','temperature',cells_per_ml) %>% pivot_wider(names_from=temperature,values_from=cells_per_ml) %>% mutate(diff=`2007`-`1952`) %>% slice_max(diff)
> ```
{: .solution}
...
> ## Solution
> ```{r,comment=NA,echo=F,collapse=T}
> filter(sample_and_taxon, temperature == 1952 | temperature == 2007) %>% select('country','temperature',cells_per_ml) %>% pivot_wider(names_from=temperature,values_from=cells_per_ml) %>% mutate(diff=`2007`-`1952`) %>% filter(diff < 0)
> ```
{: .solution}
