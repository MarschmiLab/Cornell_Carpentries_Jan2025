...
```{r FunctionsNoArgs2, eval = FALSE}
getwd()
```
```{r FunctionNoArgs3, echo = FALSE}
cat("\"/Users/augustuspendleton/Desktop/ontario-report\"")
```
...
> ```
> ```{r FunctionsComments3, eval = FALSE}
>  read_csv(file = 'sample_data.csv') # reads in csv file
> ```
> ```{r FunctionsComments4, echo = FALSE}
>  read_csv(file = 'data/sample_data.csv') # reads in csv file
> ```
{: .callout}
...
```{r ImportFullDataSolutionEval, include=FALSE}
