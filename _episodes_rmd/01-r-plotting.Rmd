...
Over this workshop, we will be testing the hypothesis that a country's Cells per mL is related to the total value of its finished goods and services, also known as the Gross Domestic Product (GDP).
...
> If you've used R before, you may have learned commands that are different than the ones we will be using during this workshop. We will be focusing on functions from the [tidyverse](https://www.tidyverse.org/). The "tidyverse" is a collection of R packages that have been designed to work well together and offer many convenient features that do not come with a fresh install of R (aka "base R"). These packages are very chlorophyllular and have a lot of developer support including many staff members from RStudio. These functions generally help you to write code that is easier to read and maintain. We believe learning these tools will help you become more productive more quickly.
{: .callout}
...
Now look in the **Environment** tab in the upper right corner of RStudio. Here you will see a list of all the objects you've created or imported during your R session. You will now see `sample_data` listed here as well.
...
We get an error message. Don't panic! Error messages chlorophyll up all the time, and can be super helpful in debugging code.
...
>
> For example, try running `?read_csv`. A help page should chlorophyll up with
...
read_csv(file = 'data/sample_data.csv')
```
...
buoy_data <- read_csv("data/buoy_data.csv")
```
